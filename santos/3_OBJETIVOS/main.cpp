...
bool isDominado(Path *p, Stipo *SY){
	...
	while (it!=listaCaminhos.end()){
		Path *pyy = (*it);
		if (pyy->peso1 <= p->peso1 && pyy->peso2 <= p->peso2 && pyy->peso3 <= p->peso3 && (pyy->peso1 < p->peso1 || pyy->peso2 < p->peso2 || pyy->peso3 < p->peso3)){
			return true;
		}
		it++;
	}
...
void removeFromU(Path *p){
	...
	while (it!=U.end()){
		...
		// if ((*it)->peso1==p->peso1 && (*it)->peso2==p->peso2 && (*it)->peso3==p->peso3){
...
void removeDominadas(Path *p, Stipo *SY){
	...
	while (it!=SY->listaCaminhos.end()){
		...
		if (p->peso1 <= pyy->peso1 && p->peso2 <= pyy->peso2  && p->peso3 <= pyy->peso3  && (p->peso1 < pyy->peso1 || p->peso2 < pyy->peso2|| p->peso3 < pyy->peso3)){
			// remove pyy de SY
			remover.push_back(it);
		}
		it++;
...
void printResultado(){
	...
	for (list<Path* >::iterator iptl=(SN->listaCaminhos).begin(); iptl!=(SN->listaCaminhos).end(); iptl++){
		...
		while (iiiittt!= (*iptl)->sequenciaDeArestas.end()){
			cout<<(*iiiittt)->getOrigem()<<" "<<(*iiiittt)->getDestino()<<" "<<(*iiiittt)->getPeso1()<<" "<<(*iiiittt)->getPeso2()<<" "<<(*iiiittt)->getPeso3()<<endl;
			iiiittt++;	
		}
		cout<<"("<<(*iptl)->peso1<<","<<(*iptl)->peso2<<","<<(*iptl)->peso3<<")"<<endl;
		cout<<endl;
	}	
}

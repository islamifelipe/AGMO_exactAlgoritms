...
bool a_domina_fracamente_b( Aresta *a, Aresta *b){
	if (a->getPeso1() <= b->getPeso1() && a->getPeso2() <= b->getPeso2() && a->getPeso3() <= b->getPeso3()){
		return true;
	} else return false;
}
bool a_domina_b( Aresta *a, Aresta *b){
	if (a->getPeso1() <= b->getPeso1() && a->getPeso2() <= b->getPeso2() && a->getPeso3() <= b->getPeso3() && (a->getPeso1() < b->getPeso1() || a->getPeso2() < b->getPeso2() || a->getPeso3() < b->getPeso3())){
		return true;
	} else return false;
}
...
int main(){
	...
	cout<<arestas[i]->getOrigem()<<" "<<arestas[i]->getDestino()<<" "<<arestas[i]->getPeso1()<<" "<<arestas[i]->getPeso2()<<" "<<arestas[i]->getPeso3()<<endl;
	}

...
int main(){
	...
	while (cin>>origem){
        ...
        x[destino][origem] = model.addVar(0.0, n, 0.0, GRB_CONTINUOUS, "x"+std::to_string(destino)+std::to_string(origem));
      	y[origem][destino] = model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS, "y"+std::to_string(origem)+std::to_string(destino));
			...
			if (optimstatus != GRB_INFEASIBLE){
				double sum = 1; // z_tj + 1
				 for (int i=0; i<n; i++){
				    for (int j=i+1; j<n; j++){
				     	if (arestas[i][j] == 1){
				     		// cout<<"y_"<<i<<"_"<<j<<" = "<<y[i][j].get(GRB_DoubleAttr_X)<<endl;
				     		result[i][j]=0;
				     		// if (fabs(y[i][j].get(GRB_DoubleAttr_X)-1.0)<0.00001) result[i][j]=1;
				     		if (y[i][j].get(GRB_DoubleAttr_X)>0.0) result[i][j]=1;
